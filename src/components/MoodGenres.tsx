...
type ItemSliderProps = {
  title: string;
  sliderData: ExploreData[];
};

const Mood: FunctionComponent<ItemSliderProps> = ({ title, sliderData }) => {
  const sliderRef = useRef<HTMLDivElement>(null);
  const scrollAmount = 300;
  const [data, setData] = useState<ExploreData[]>(sliderData || []);
  ...
  return (
    <div className="overflow-x-hidden">
      ...
      <div className="">
        ...
        >
          {data.length > 0 ? (
            data.map((item: ExploreData, i: number) => (
